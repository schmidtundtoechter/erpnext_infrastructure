...
function local.install_update() {
  ...
  if [ ! -d /home/frappe/frappe-bench/apps/ersteingabe_lead ]; then
      echo "App ersteingabe_lead does not exist - cloning";
      GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no" bench get-app git@github.com:larsmaeurer/ersteingabe_lead.git;
  else
      echo "App ersteingabe_lead already exists - pulling latest changes";
      cd /home/frappe/frappe-bench/apps/ersteingabe_lead;
      GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no" git pull;
  fi;
  rm -rf ~/.ssh/id_rsa

  echo "Install ersteingabe_lead app and migrate";
  bench --site $SITE install-app ersteingabe_lead;
